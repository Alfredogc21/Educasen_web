...
<body>
  ...
  <nav role="navigation">
    <div class="nav-wrapper">
      ...
      <ul class="right hide-on-med-and-down">
        ...
        <li><a class="sidenav-close" href="introduccionNaturales.php">Regresar</a></li>
      </ul>
    </div>
  </nav>
  ...
  <!------------------------------------------------------------------------------------------------------------>
  <h1 class="titulo">Prueba de Inglés</h1>
  <?php if ($errores) : ?>
    <div class="error">
      <?php echo $errores; ?>
    </div>
  <?php else : ?>
    <div class="contenedor">
      ...
      <img class="imagenEnunciadoPregunta materialboxed" src="views/imagenes/fotos/<?php echo $resultadoImagen[0]['imagen'] = ($resultadoImagen == false) ? "" : $resultadoImagen[0]['imagen']; ?>" alt="Imagen de la pregunta">
      <div class="contenedor__Opciones">
        ...
        <form class="contenidoOpciones" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" name="preguntas">
          <?php
          // Recorre las respuestas de texto
          foreach ($resultadoRespuestas as $fila) :
            if ($fila['tipo_contenido_id'] == 1) : ?>
              <p>
                <label>
                  <input class="with-gap" type="radio" name="respuesta" value="<?php echo $fila['validacion_pregunta_id']; ?>" />
                  <span class="enunciadoSpan"><?php echo $fila['contenido_respuestas']; ?><br></span>
                </label>
              </p>
          <?php endif;
          endforeach; ?>
          <fieldset class="contenedor__imgrespuestas">
          <?php
          // Recorre las respuestas de imágenes
          foreach ($resultadoRespuestas as $fila) :
            if ($fila['tipo_contenido_id'] == 2) : ?>
