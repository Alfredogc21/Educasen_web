...
if (isset($_SESSION['usuarios'])) {
    ...
    $sqlPreguntas =$conexion->prepare('SELECT p.* FROM preguntas p JOIN usuarios u JOIN opcion_pregunta op LEFT JOIN calificacion c ON c.preguntas_id = p.id AND c.usuarios_id = u.id WHERE u.id = :idUsuario AND p.opcion_pregunta_id = :opcionPregunta AND p.materia_id = :materia AND c.id is null LIMIT 1;');
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        //recargar la pagina
        header('Refresh: 0.1 ; URL=examenIngles.php');
    }

    if($resultadoUsername['roles_id'] == 2){ // Si es estudiante
        require 'views/examenIngles.view.php';
